...
public class operation_dependency_tests : IntegrationContext
{
    ...
    public async Task soft_delete_and_tombstone_in_same_save()
    {
        ...
        await using var query = theStore.QuerySession();
        var loaded = await query.LoadAsync<User>(doc.Id, TestContext.Current.CancellationToken);
        loaded.ShouldBeNull();
    ...
    public async Task store_multiple_types_with_events_all_committed()
    {
        ...
        await using var query = theStore.QuerySession();
        var user = await query.LoadAsync<User>(userId, TestContext.Current.CancellationToken);
        user.ShouldNotBeNull();

        var loadedString = await query.LoadAsync<StringDoc>(stringDoc.Id, TestContext.Current.CancellationToken);
