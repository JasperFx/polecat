...
public class live_aggregation_tests : IntegrationContext
{
    ...
    public async Task can_aggregate_stream_from_start()
    {
        ...
        var party = await query.Events.AggregateStreamAsync<QuestParty>(streamId, token: TestContext.Current.CancellationToken);
    ...
    {
        await using var query = theStore.QuerySession();
        var party = await query.Events.AggregateStreamAsync<QuestParty>(Guid.NewGuid(), token: TestContext.Current.CancellationToken);
    ...
    public async Task aggregate_with_version_cutoff()
    {
        ...
        var party = await query.Events.AggregateStreamAsync<QuestParty>(streamId, version: 2, token: TestContext.Current.CancellationToken);
    ...
    public async Task aggregate_with_existing_state()
    {
        ...
        var party = await query.Events.AggregateStreamAsync<QuestParty>(streamId,
            state: existingState, fromVersion: 2, token: TestContext.Current.CancellationToken);
    ...
    public async Task aggregate_does_not_persist()
    {
        ...
        var loaded = await query2.LoadAsync<QuestParty>(streamId, TestContext.Current.CancellationToken);
        loaded.ShouldBeNull();
    }
}
